...
def hearth_contract() -> ModelerIntegrationContract:
    ...
    safe_actions = [
        ContractAction(
            ...
            risk_level="low",
            endpoint="/integration/hearth/status",
            mcp_tool="modeler_status",
            status="available",
            requires_approval=False,
            description="Report whether the local Modeler service is reachable, compatible, degraded, or unavailable.",
            allowed_inputs=["optional Hearth request/correlation ID"],
            response_guarantees=["includes service status", "includes contract version", "does not expose secrets"],
        ),
        ContractAction(
            id="question.answer",
